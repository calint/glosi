  ...
  public:
    ...
    inline auto are_in_collision_with_sphere(glm::vec3 const& position,
        ...
        return std::ranges::all_of(
            world_planes, [&point, radius](glm::vec4 const& plane) {
    ...
    //     // project both the sphere and the convex volume onto the plane
    //     normal size_t const n = world_planes.size(); for (uint32_t i = 1; i <
    //     n; ++i) {
    //       glm::vec3 const &point = world_points[i];
    ...
    //   float volume_min_projection = glm::dot(glm::vec3{world_points[0]},
    //   axis); float volume_max_projection = volume_min_projection; size_t
    //   const n = world_points.size(); for (uint32_t i = 1; i < n; ++i) {
