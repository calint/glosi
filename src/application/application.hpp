...
static inline auto application_init() -> void {
    ...
    glob_ix_bullet =
        glos::globs.load("assets/obj/asteroids/ship_bullet.obj",
    ...
    // float const skydome_scale = length(glm::vec2{grid_size / 2, grid_size /
    // 2}); skydome->bounding_radius = skydome_scale; skydome->scale =
    // {skydome_scale, skydome_scale, skydome_scale};
...
static inline auto application_on_update_done() -> void {
    ...
    // note: -0.00001f because of look_at does not work properly when x and z
    // are
    //       equal
